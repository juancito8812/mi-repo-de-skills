...
## When to use
- Build or run `llama-server` / `llama-cli` directly
- Convert and quantize GGUF models, pick the right quant for hardware
- Discover GGUF repos on Hugging Face Hub
- Enumerate `.gguf` files and reconstruct launch commands
- Profile Windows / macOS / Linux hardware and match tiers

## Windows shortcut: use Ollama first
On Windows, compiling or installing `llama.cpp` directly is often harder than using Ollama. Prefer Ollama as the local backend whenever possible — it ships the same GGUF runtime and exposes an OpenAI-compatible API out of the box.

See `references/windows-ollama-setup.md` for:
- Portable ZIP install when MSI fails (error 1620 / 2203)
- `ollama serve` detached service pattern
- Auto-start scheduled task recipe
- One-time Hermes config edits to switch to Ollama
...
- **[hardware-matching.md](references/hardware-matching.md)** — profile Windows/macOS/Linux hardware, match RAM/CPU/GPU to model tiers, Ollama/llama.cpp commands by tier, AVX-512 guidance, Windows Defender pitfalls
